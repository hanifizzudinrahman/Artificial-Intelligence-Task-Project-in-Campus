...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> IMPORT DATASET
dataset = pd.read_csv('C:\Hanif Izzudin Rahman D4 EB 2016\S2 Elektro 2020 - PENS\Semester 1\Artificial Intelligent -  Aliridho\Tugas\AI_M10_MultiPerceptron_Heart\heartV2.csv')
dataset
...
for i in range(1, epoch):    
    for j in range (1, 270+1):                # 270 = Jumlah dataset (270 input)
        ...
        # =================================================================== Input Layer 
        for k in range (1, 13+1):                 # 1,3 = 2 - Jumlah hidden layer
            H[k] = Bias*w[0][k]
        ...
        H[0] = Bias
        for k in range (13+1):          # 3 = Jumlah Hidden layer (Bias(H0), H1 dan H2)
            dwH[k] = miu*H[k]*dC
            # Update Weight
            wh[k] = wh[k] + dwH[k]            
        ...
        for k in range (1, 13+1):     # 1, 13+1 = 13 (Jumlah Input)
            for m in range (1, 13+1):     # 1,3 = 2 (Jumlah Hidden Layer)
...
for j in range (1, 270+1):                # 270 = Jumlah dataset (270 input)
    print("%.2f "*13 %(I[j][1], I[j][2], I[j][3], I[j][4], I[j][5], I[j][6], I[j][7], I[j][8], I[j][9], I[j][10], I[j][11], I[j][12], I[j][13]), " || Output= %.9f" %(Cout[j]), " || CoutTh= ", CoutTh[j], " || Out Real = ", Out[j])
